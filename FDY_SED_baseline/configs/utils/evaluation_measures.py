...
def event_based_evaluation_df(
    reference, estimated, t_collar=0.500, percentage_of_length=0.5 # default was tcollar = 0.2 changed to fit the data 
):                                          # default was percentage_of_length = 0.2 changed to fit the data
...
def segment_based_evaluation_df(reference, estimated, time_resolution=0.5): # default was time_resolution = 1.0 changed to fit the data
...
def compute_per_intersection_macro_f1(
    ...
    """
    gt = pd.read_csv(ground_truth_file, sep="\t", header=None, names=["filename", "onset", "offset", "event_label"])
    durations = pd.read_csv(durations_file, sep="\t", header=None, names=["filename", "duration"])
