...
# def get_labeldict(): original line as Dcase data set has 10 classes
...
def get_labeldict(): #for my custom data set has 3 classes
    return OrderedDict({
        "Bell": 0,
        "Hiss": 1,
        "Rattle": 2
...
def get_mt_datasets(configs, server_cfg, train_cfg):
    ...
    #synth_train_df = pd.read_csv(synth_train_tsv, sep="\t") original line
    synth_train_df = pd.read_csv(
        synth_train_tsv,
        sep="\t",
        header=None,
    ...
    #weak_df = pd.read_csv(dataset_cfg["weak_tsv"], sep="\t") original line
    weak_df = pd.read_csv(
        dataset_cfg["weak_tsv"],
        sep="\t",
        header=None,
        names=["filename", "event_labels"]
    )
    ...
    #synth_valid_df = pd.read_csv(synth_valid_tsv, sep="\t") original line
    synth_valid_df = pd.read_csv(
        synth_valid_tsv,
        sep="\t",
        header=None,
        names=["filename", "onset", "offset", "event_label"]
    ...
    if not general_cfg["test_on_public_eval"]:
        ...
        #test_df = pd.read_csv(test_tsv, sep="\t") original line
        test_df = pd.read_csv(
            test_tsv,
            sep="\t",
            header=None,
...
def get_f1calcs(n_class, device):
    stud_f1calc = torchmetrics.classification.F1Score(num_labels=n_class, average="macro", task="multilabel")
    tch_f1calc = torchmetrics.classification.F1Score(num_labels=n_class, average="macro", task="multilabel")
